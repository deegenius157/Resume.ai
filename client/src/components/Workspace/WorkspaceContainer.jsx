...
export const WorkspaceContainer = ({ onNavigate, initialData, currentUser }) => {
  ...
  const [activeSubTab, setActiveSubTab] = useState(null); // 'template', 'accent', or null
  ...
  const [localResume, setLocalResume] = useState(currentResume);

  useEffect(() => {
    setLocalResume(currentResume);
  }, [currentResume]);

  const updateLocalPersonalInfo = (info) => {
    setLocalResume((prev) => ({
      ...prev,
  ...
  const updateLocalSummary = (summary) => {
    setLocalResume((prev) => ({ ...prev, summary }));
  };

  const addLocalItem = (field, item) => {
    setLocalResume((prev) => ({
      ...prev,
  ...
  const updateLocalItem = (field, index, data) => {
    setLocalResume((prev) => {
  ...
  const removeLocalItem = (field, index) => {
    setLocalResume((prev) => ({
      ...prev,
  ...
  const handleAddNew = (type) => {
    ...
    addLocalItem(type.toLowerCase(), templates[type.toLowerCase()]);
  };
  ...
  const saveResume = async (showNotification = true) => {
    ...
    const hasValidUuid = isUuid(localResume.id);
    const resumeId = hasValidUuid ? localResume.id : null;
    ...
    const savedRecord = data && data[0];
    const newId = savedRecord ? savedRecord.id : resumeId;

    const finalResumeRecord = {
      ...resumeRecord,
      id: newId
    };

    setLocalResume(finalResumeRecord);
    setCurrentResume(finalResumeRecord);
  ...
  const handleSaveCurrentTabData = async () => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      // Commit the absolute latest resume data state to the database before export
    ...
    <div className="h-screen w-screen flex bg-slate-50 overflow-hidden text-slate-800 font-sans" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div className={`flex-1 bg-white p-6 md:p-12 overflow-y-auto shadow-inner border-r border-slate-100 pb-24 lg:pb-12 ${mobileView === 'edit' ? 'flex flex-col' : 'hidden lg:flex lg:flex-col'}`}>
        <div className="max-w-2xl mx-auto w-full">
          ...
          <div className="flex items-center justify-between mb-12 pb-6 border-b border-slate-100">
            ...
            {activeTab === 'Summary' && (
              <button
                ...
                onClick={() => handleAIEnhance(localResume?.summary, 'summary', updateLocalSummary)}
          ...
          <div className="flex items-center justify-between bg-slate-50 p-4 rounded-3xl border border-slate-100 mb-6 relative overflow-x-auto whitespace-nowrap scrollbar-none gap-4">
            <div className="flex items-center gap-2 shrink-0">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-2">Theme</span>
            </div>
            
            <div className="flex items-center gap-2 relative shrink-0">
              <button
                onClick={() => {
                  setActiveSubTab(prev => prev === 'template' ? null : 'template');
                }}
              ...
              <button
                ...
                onClick={() => {
                  setActiveSubTab(prev => prev === 'accent' ? null : 'accent');
                }}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer ${
                  activeSubTab === 'accent' 
                    ? 'bg-slate-900 text-white shadow-lg shadow-slate-950/20' 
          ...
          {/* Sub-tab options container rendered below the tab row, preventing overflow clipping */}
          {activeSubTab === 'accent' && (
            <div className="bg-white border border-slate-100 rounded-[2rem] p-6 shadow-2xl z-50 mb-8 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="flex justify-between items-center mb-4">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Select Accent Color</span>
                <button onClick={() => setActiveSubTab(null)} className="text-slate-300 hover:text-slate-600 transition-colors cursor-pointer">
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-4 py-4 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all border cursor-pointer ${
                        isActive 
            ...
            {activeTab === 'Personal Info' && (
              <div className="space-y-8">
                <div className="flex flex-col gap-3 mb-8">
                  ...
                  >
                    <input id="photo-upload" type="file" className="hidden" accept="image/*" onChange={(e) => { const file = e.target.files[0]; if (file) { const reader = new FileReader(); reader.onloadend = () => updateLocalPersonalInfo({ photo: reader.result }); reader.readAsDataURL(file); } }} />
                    <div 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    ...
                    <input type="text" name="fullName" value={localResume?.personalInfo?.fullName || ''} onChange={(e) => updateLocalPersonalInfo({ fullName: e.target.value })} placeholder="John Snow" className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-xl px-5 py-3.5 text-xs font-bold outline-none focus:border-[#10B981] focus:ring-1 focus:ring-[#10B981] transition shadow-sm" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Job Title</label>
                    <input type="text" value={localResume?.personalInfo?.profession || ''} onChange={(e) => updateLocalPersonalInfo({ profession: e.target.value })} placeholder="Lead Designer" className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-xl px-5 py-3.5 text-xs font-bold outline-none focus:border-[#10B981] focus:ring-1 focus:ring-[#10B981] transition shadow-sm" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    ...
                    <input type="email" value={localResume?.personalInfo?.email || ''} onChange={(e) => updateLocalPersonalInfo({ email: e.target.value })} placeholder="hello@domain.com" className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-xl px-5 py-3.5 text-xs font-bold outline-none focus:border-[#10B981] focus:ring-1 focus:ring-[#10B981] transition shadow-sm" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Contact Line</label>
                    <input type="tel" value={localResume?.personalInfo?.phone || ''} onChange={(e) => updateLocalPersonalInfo({ phone: e.target.value })} placeholder="+1 (555) 000-0000" className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-xl px-5 py-3.5 text-xs font-bold outline-none focus:border-[#10B981] focus:ring-1 focus:ring-[#10B981] transition shadow-sm" />
                  </div>
                </div>

                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Current Base</label>
                  <input type="text" value={localResume?.personalInfo?.location || ''} onChange={(e) => updateLocalPersonalInfo({ location: e.target.value })} placeholder="San Francisco, CA" className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-xl px-5 py-3.5 text-xs font-bold outline-none focus:border-[#10B981] focus:ring-1 focus:ring-[#10B981] transition shadow-sm" />
                </div>
                <SaveActionBar />
              </div>
            )}
                  ...
                  <div className="space-y-8">
                    {localResume.experience.map((exp, index) => (
                      <div key={exp.id} className="p-8 bg-white border border-slate-100 rounded-[2rem] relative group flex flex-col gap-6 shadow-xl shadow-slate-200/40 hover:border-[#10B981]/30 transition-all">
                        <button onClick={() => removeLocalItem('experience', index)} className="absolute top-6 right-6 p-2 text-slate-300 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100"><Trash2 size={16} /></button>
                        <div className="grid grid-cols-2 gap-6">
                          <div className="flex flex-col gap-2">
                            <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Organization</label>
                            <input type="text" placeholder="Apple Inc." value={exp.company} onChange={(e) => updateLocalItem('experience', index, { company: e.target.value })} className="bg-slate-50 border-none rounded-xl px-5 py-3 text-xs outline-none font-bold focus:ring-2 focus:ring-[#10B981]/20" />
                          </div>
                          <div className="flex flex-col gap-2">
                            <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Position</label>
                            <input type="text" placeholder="Senior Analyst" value={exp.title} onChange={(e) => updateLocalItem('experience', index, { title: e.target.value })} className="bg-slate-50 border-none rounded-xl px-5 py-3 text-xs outline-none font-bold focus:ring-2 focus:ring-[#10B981]/20" />
                          </div>
                        </div>
                        <div className="flex flex-col gap-2">
                          <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Tenure</label>
                          <input type="text" placeholder="Jan 2020 — Present" value={exp.dates} onChange={(e) => updateLocalItem('experience', index, { dates: e.target.value })} className="bg-slate-50 border-none rounded-xl px-5 py-3 text-xs outline-none font-bold focus:ring-2 focus:ring-[#10B981]/20" />
                        </div>
                        <div className="flex flex-col gap-3">
                          ...
                          <textarea placeholder="Synthesize your contributions..." value={exp.description} onChange={(e) => updateLocalItem('experience', index, { description: e.target.value })} className="bg-slate-50 border-none rounded-[1.5rem] px-5 py-4 text-sm outline-none font-medium resize-none h-40 focus:ring-2 focus:ring-[#10B981]/20 leading-relaxed" />
                        </div>
                      </div>
                    ))}
                  </div>
                )}
                {localResume.experience.length > 0 && <SaveActionBar />}
              </div>
            )}
                  ...
                  <div className="space-y-6">
                    {localResume.education.map((edu, index) => (
                      <div key={edu.id} className="p-8 bg-white border border-slate-100 rounded-[2rem] relative group flex flex-col gap-6 shadow-xl shadow-slate-200/40 hover:border-[#10B981]/30 transition-all">
                        <button onClick={() => removeLocalItem('education', index)} className="absolute top-6 right-6 p-2 text-slate-300 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100"><Trash2 size={16} /></button>
                        <div className="grid grid-cols-2 gap-6">
                          <div className="flex flex-col gap-2">
                            <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Institution</label>
                            <input type="text" placeholder="Harvard University" value={edu.school} onChange={(e) => updateLocalItem('education', index, { school: e.target.value })} className="bg-slate-50 border-none rounded-xl px-5 py-3 text-xs outline-none font-bold focus:ring-2 focus:ring-[#10B981]/20" />
                          </div>
                          <div className="flex flex-col gap-2">
                            <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Certification</label>
                            <input type="text" placeholder="MS in Data Science" value={edu.degree} onChange={(e) => updateLocalItem('education', index, { degree: e.target.value })} className="bg-slate-50 border-none rounded-xl px-5 py-3 text-xs outline-none font-bold focus:ring-2 focus:ring-[#10B981]/20" />
                          </div>
                        </div>
                        <div className="flex flex-col gap-2">
                          <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Completion</label>
                          <input type="text" placeholder="May 2021" value={edu.dates} onChange={(e) => updateLocalItem('education', index, { dates: e.target.value })} className="bg-slate-50 border-none rounded-xl px-5 py-3 text-xs outline-none font-bold focus:ring-2 focus:ring-[#10B981]/20" />
                        </div>
                      </div>
                    ))}
                  </div>
                )}
                {localResume.education.length > 0 && <SaveActionBar />}
              </div>
            )}
              ...
              <div className="space-y-8">
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex gap-3">
                    <input id="skill-add" type="text" placeholder="Identify an expertise..." onKeyDown={(e) => { if (e.key === 'Enter' && e.target.value) { addLocalItem('skills', { name: e.target.value }); e.target.value = ''; } }} className="flex-1 bg-slate-50 border-none rounded-2xl px-6 py-4 text-sm outline-none font-black text-slate-700 shadow-inner focus:ring-2 focus:ring-[#10B981]/20" />
                    <button onClick={() => { const inp = document.getElementById('skill-add'); if (inp.value) { addLocalItem('skills', { name: inp.value }); inp.value = ''; } }} className="bg-[#10B981] hover:bg-[#0E9F6E] text-white px-10 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-emerald-900/10 transition-all active:scale-95">Verify & Add</button>
                  </div>
                </div>
                <div className="flex flex-wrap gap-3 p-2">
                  {localResume.skills.map((skill, index) => (
                    <div key={skill.id} className="flex items-center gap-3 bg-slate-900 text-white pl-5 pr-3 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-[#10B981] transition-all hover:-translate-y-1">
                      {skill.name}
                      <button onClick={() => removeLocalItem('skills', index)} className="p-1 text-slate-500 hover:text-white transition-all"><Trash2 size={14} /></button>
                    </div>
                  ))}
                </div>
                <SaveActionBar />
      ...
      <div className={`bg-slate-100 flex-col shrink-0 relative border-l border-slate-200 pb-24 lg:pb-0 ${mobileView === 'preview' ? 'flex w-full' : 'hidden lg:flex lg:w-[48%]'}`}>
        ...
        <div className="flex-1 p-12 overflow-y-auto flex justify-center items-start">
          <div id="resume-canvas" ref={resumeCanvasRef} className="w-full max-w-[210mm] min-h-[297mm] bg-white shadow-2xl p-14 rounded-sm border border-slate-200 flex flex-col font-sans relative overflow-hidden">
            ...
            <div className="mt-auto pt-16 text-center"><div className="inline-block px-6 py-2 bg-slate-50 rounded-full text-[9px] font-black tracking-[0.3em] text-slate-400 uppercase italic">Engineered by Genusjob Resume AI</div></div>
          </div>
        </div>
      </div>
