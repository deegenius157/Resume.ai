...
export default function SelectionHub({ setCurrentView, currentUser, setView, setUploadedData }) {
  ...
  const userName = currentUser?.user_metadata?.full_name || currentUser?.fullName || currentUser?.name || "User";
  ...
  const loadResumes = async () => {
    ...
    if (error) {
      console.error("Error loading saved resumes from database:", error);
    } else {
      const mapped = (data || []).map(row => ({
        id: row.id,
        title: row.resume_data?.personalInfo?.fullName
          ? `${row.resume_data.personalInfo.fullName}'s Resume`
          : 'Untitled Resume',
  ...
  return (
    <div className="min-h-screen bg-white text-slate-950 flex flex-col" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <header className="border-b border-slate-100 px-12 py-4 flex items-center justify-between bg-white">
        ...
        >
          <span className="text-xl font-extrabold tracking-tight text-slate-900 font-sans">GENUS</span>
          <span className="text-xl font-medium tracking-tight text-[#10B981] font-sans">JOB.COM</span>
        </a>
                  ...
                  <div key={resume.id} className="relative group w-44 h-56 bg-white border border-slate-100 rounded-lg shadow-[0_4px_20px_rgba(0,0,0,0.02)] hover:shadow-[0_4px_25px_rgba(0,0,0,0.06)] hover:border-slate-200 flex flex-col items-center justify-center gap-3 transition text-center cursor-pointer">
                    ...
                    <div
                      onClick={() => {
                        ...
                        // Inject unique database id to the payload object
                        const payloadWithId = {
                          ...payload,
                          id: resume.id
                        };
                        setUploadedData(payloadWithId);
                        setCurrentView('workspace');
                      }}
